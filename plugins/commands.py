...
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, REQST_CHANNEL, SUPPORT_CHAT_ID, MAX_B_TN, CHANNEL_ID, CHANNEL_LINK
...
async def start(client, message):
    ...
    if not files_:
        ...
        try:
            ...
            await message.reply(script.FILE_READY_TXT.format(query.from_user.mention, title, size),
            True,
...
async def settings(client, message):
    ...
    if settings is not None:
        ...
        btn = [[
                InlineKeyboardButton("🛡️Oᴘᴇɴ Hᴇʀᴇ🛡️", callback_data=f"opnsetgrp#{grp_id}"),
